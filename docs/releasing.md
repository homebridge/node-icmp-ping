...
Musl builds use the official Rust 1.98.1 and Node 24 Alpine 3.23 images on matching native runners (`.github/musl.Dockerfile`), with dynamic musl linkage (`-C target-feature=-crt-static`) for the addon. Host-side Actions avoid requiring glibc-dependent JavaScript actions inside Alpine. Clean install tests use the unmodified Node images without Rust or build tools.
